...
for sensor in ["accel_", "gyro_", "CF_angle_", "kalman_"]:
...
if not os.path.exists(filename):
    print('Folder does not exist! Creating folder "' + str(filename) + '"...')
    os.mkdir(filename + '/')
starting_index = int(input("What number should we start on? "))
    
duration_s = float(input("Please input how long should a sensor trace be in seconds (floats are OK): "))
...
while True:
    ...
    while elapsed_ms < duration_s * 1000:
        ...
        head = [heading, tiltCompensatedHeading]

        row = [elapsed_ms, int(elapsed_ms - previous_elapsed_ms)]
        row += accel + gyro + cf_angle + kalman + head
    ...
    print("Finished collecting file " + str(i) + "!" )
    i += 1
